--deleting any entity with 'FAO' in its name

delete from KeyGroupYeilds
where Entity like '%FAO%'


-- The average tonnes per hectar for what throughout the years entities have produced
...
--Top 5 entities for the production of rice

select top 5 Entity, SUM([Rice in tonnes per hectar]) as OverallRiceProduction
from KeyGroupYeilds
group by Entity
...
-- Each entity vs all other countries for percentage of yeilds
...
-- Max production year and the production value
...
-- getitng the max and min yield by each entity and their years and creating a table for it
